...
const ForgetPasswordPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      dispatch(forgetpassword(values, csrfToken))
    } catch (error) {
        ...
        <Card sx={{ zIndex: 1 }}>
          <CardContent sx={{ padding: theme => `${theme.spacing(12, 9, 7)} !important` }}>
            ...
            <form noValidate autoComplete='off' onSubmit={handleSubmit}>
              {loading ? <Loader /> : null}
              <TextField fullWidth type='email' onChange={handleChange('username')} label='username' sx={{ marginBottom: 4 }} />
              <FormControlLabel
                control={<Checkbox />}
                label={
                  <Fragment>
...
ForgetPasswordPage.getLayout = page => <BlankLayout>{page}</BlankLayout>
